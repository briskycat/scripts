...
jar_file () {
  echo "$script_dir/.lib/$2/sbt-launch.jar"
}
...
addSbt () {
  dlog "[addSbt] arg = '$2'"
  sbt_commands=( "${sbt_commands[@]}" "$2" )
}
addScalac () {
  dlog "[addScalac] arg = '$2'"
  scalac_args=( "${scalac_args[@]}" "$2" )
}
addResidual () {
  dlog "[residual] arg = '$2'"
  residual_args=( "${residual_args[@]}" "$2" )
}
addResolver () {
  addSbt "set resolvers in ThisBuild += $2"
}
addDebugger () {
  addJava "-Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=$2"
}
get_jvm_opts () {
  ...
  while [[ $# -gt 0 ]]; do
    ...
    -v|-verbose) verbose=1 && shift ;;
      ...
      -q|-quiet) quiet=1 && shift ;;
           ...
           -ivy) require_arg path "$2" "$2" && addJava "-Dsbt.ivy.home=$2" && shift 2 ;;
           -mem) require_arg integer "$2" "$2" && sbt_mem="$2" && shift 2 ;;
     -no-colors) addJava "-Dsbt.log.noformat=true" && shift ;;
      -no-share) addJava "$noshare_opts" && shift ;;
      -sbt-boot) require_arg path "$2" "$2" && addJava "-Dsbt.boot.directory=$2" && shift 2 ;;
       -sbt-dir) require_arg path "$2" "$2" && sbt_dir="$2" && shift 2 ;;
     -debug-inc) addJava "-Dxsbt.inc.debug=true" && shift ;;
       -offline) addSbt "set offline := true" && shift ;;
     -jvm-debug) require_arg port "$2" "$2" && addDebugger $2 && shift 2 ;;
         -batch) exec </dev/null && shift ;;

    -sbt-create) sbt_create=true && shift ;;
  -sbt-snapshot) sbt_explicit_version=$sbt_snapshot_version && shift ;;
       -sbt-jar) require_arg path "$2" "$2" && sbt_jar="$2" && shift 2 ;;
   -sbt-version) require_arg version "$2" "$2" && sbt_explicit_version="$2" && shift 2 ;;
 -scala-version) require_arg version "$2" "$2" && addSbt "scalaVersion := \"$2\"" && shift 2 ;;
    -scala-home) require_arg path "$2" "$2" && addSbt "set scalaHome in ThisBuild := Some(file(\"$2\"))" && shift 2 ;;
     -java-home) require_arg path "$2" "$2" && java_cmd="$2/bin/java" && shift 2 ;;
